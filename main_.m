...
noise = 0.95*max(input_signal(:,1))*randn(length(input_signal),1);  %生成高斯噪声并限幅
input_with_noise = input_signal + noise; %添加噪声后的信号
audiowrite('LMSrefns.wav',noise,44100) ;   %输出模拟噪声文件
audiowrite('LMSprimsp.wav',input_with_noise,44100);  %输出模拟主语音信号

%STEP3:主麦克风（含噪声）的语音信号的读取并构建时域和频域图像
[primary,fs]=audioread('LMSprimsp.wav');
n=length(primary) ; 
...
[fref,fs]=audioread('LMSrefns.wav');
n=length(fref) ; 
F=fft(fref,n);  
figure;
subplot(2,1,1);
...
audiowrite('LMS_filted.wav',yn,44100);  %生成语音文件LMS_filted.wav格式
